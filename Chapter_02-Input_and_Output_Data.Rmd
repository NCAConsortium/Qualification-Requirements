...
These are the concentration-time profile and dosing data that are actually analyzed by the NCA function. 
...
* Observation/Sample inputs (the concentration measurement and time point vectors have to be of equal length):
    ...
    * Vectors of time points for concentration measurements:
        * Nominal sampling time or nominal start time of sampling interval since first dose (really required??? Not needed or used for NCA, right?)
        * Nominal sampling time or nominal start time of sampling interval since most recent dose (aka reference dose) for the profile
        * Actual sampling time or actual start time of sampling interval since first dose (really required??? Not needed or used for NCA, right?)
        * Actual sampling time or actual start time of sampling interval since most recent dose (aka reference dose) 
     * Vectors of time points indicating the end of a sampling interval (for example, for urine studies)      
        * Nominal end time of sampling interval since first dose (really required??? Not needed or used for NCA, right?)
        * Nominal end time of sampling interval since most recent dose (aka reference dose) for the profile
        * Actual end time of sampling interval since first dose (really required??? Not needed or used for NCA, right?)
        * Actual end time of sampling interval since most recent dose (aka reference dose) 
* Dosing inputs:
    * Dose definition:
    ...
    * Dose time inputs (all are zero for single-dose):
        * Nominal time point or nominal start time of reference dose (should always be zero, right???)
        * Actual time point or actual start time of reference dose
        * Nominal duration of dose administration for intravascular infusion
...
Outputs should align with the SDTM PP domain. MORE TO COME ...
