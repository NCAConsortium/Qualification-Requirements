...
* (Optional, default is ???) Value that indicates how to handle loq-values (i.e., what BQL rule should be applied)
* (Optional, default is ???) Value that indicates how to handle pre-dose samples (i.e., what pre-dose rule should be applied)
* Partial AUC ranges (the vectors for the start and end values of the partial AUC rangs have to be of equal length)
    * (Optional) Vector of the start values for time ranges for partial AUC calculation (e.g., 0 and 12 for AUC0-24 and AUC12-24);
    * (Optional) Vector of the end values for time ranges for partial AUC calculation (e.g., 24 and 24 for AUC0-24 and AUC12-24);
...
## SDTM Alignment (not yet updated)
